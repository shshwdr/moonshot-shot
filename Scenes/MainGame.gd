...
func change_occupy(old_index_position,new_index_position):
	if not index_to_human_map.has(old_index_position) :
		push_error("human does not exist in %s"%String(old_index_position))
	if index_to_human_map.has(new_index_position) :
		push_error("human do exist in %s"%String(new_index_position))
	...
	print(index_to_human_map)
	

func remove_occupy(index_position):
	if not index_to_human_map.has(index_position) :
		push_error("human does not exist in %s"%String(index_position))
...
func _ready():
	timer.wait_time = Utils.wait_time*2
	Utils.maingame = self
